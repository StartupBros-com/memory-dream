...
| `AUDIT_MAX_INDEX_BYTES` | 32768 | Repo-hygiene audit ceiling (distinct from the loader cap — roomier, informational). | Corpus-calibrated. | Audit nags well before the loader cap actually matters. | Audit stays quiet on an index already well past the load cap. | `audit.py --max-index-bytes`, env, or config. |
| `AUDIT_MAX_INDEX_LINES` | 400 | Same, for line count. | Corpus-calibrated. | Same direction as above. | Same direction as above. | `audit.py --max-index-lines`, env, or config. |
| `AUDIT_STALE_DAYS` | 90 | Age threshold for the audit's stale-content finding class. | Corpus-calibrated. | Recently-touched notes flag as stale prematurely. | Genuinely stale notes escape the finding entirely. | `audit.py --stale-days`, env, or config. |
